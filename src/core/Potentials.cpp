...
namespace Potentials {
	...
	MixedGeometryHartree::MixedGeometryHartree(size_t nPts, size_t minPos, size_t maxPos, size_t surfPos, double maskLength, double shieldLength, int neumannSide, double dx, double mRTheta, const double* hRad, const double* rho0, const double* j0, size_t refPoint, bool includeVectorPotential) :
		nPts(nPts), minPos(minPos), maxPos(maxPos), dx(dx), mRTheta(mRTheta), refPoint(refPoint), includeVectorPotential(includeVectorPotential), neumSide(neumannSide)
	{
		...
		maskGradient = (double*) sq_malloc(sizeof(double)*nPts);
		lostCharge = (double*) sq_malloc(sizeof(double)*nPts);
		lcTemp = (double*) sq_malloc(sizeof(double)*nPts);
		...
		vtls::firstDerivative(nPts, maskProfile, maskGradient, dx);
		std::fill_n(lostCharge, nPts, 0.0);
	...
	void MixedGeometryHartree::calcPot(const double* rho, const double* cur, double* targ, double t, bool virt) {
		double dt = t - t0;
		...
		// add lost charge, accumulate according to current
		vtls::addArrays(nPts, lostCharge, drho);
		vtls::seqMulArrays(nPts, maskGradient, dcur, lcTemp);
		vtls::scaMulAddArrays(nPts, -dt, lcTemp, lostCharge);
	...
	MixedGeometryHartree::~MixedGeometryHartree(){
		sq_free(vld);
		sq_free(vd);
		sq_free(vud);
		sq_free(vud2);
