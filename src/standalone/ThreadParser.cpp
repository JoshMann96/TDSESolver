...
int ThreadParser::readCommand() {
	...
	if (curLine[0] == '#') {}
		...
		sim->addMeasurer(new Measurers::ElectronNumber(sim->getNElec(), fol)); //add electron number measurer
...
int ThreadParser::generalSimInit() {
	//TODO: rewrite to use block parameters
	double minX = 0;
	double maxX = 0;
	double maxT = 0;
	double dx = 0;
	...
	sim = new SimulationManager(n, dx, dt, maxT, callbackFunc);
	x = new double[n];
